...
//    @ObservedObject private var viewModel = CourseReviewViewModel()
    @ObservedObject private var viewModel = ProfessorReviewViewModel()
    ...
    var body: some View {
        List {
            ForEach(viewModel.reviews) { review in
                if(professor.email == review.profEmail) {
                HStack(spacing:0) {
                    Button(action: {
                        print("!")
                    }) {
                    ...
                    VStack(alignment: .leading){
                        HStack {
                            ...
                            Text("\(review.timeDate)".prefix(10))
        ...
        VStack {
            ...
            HStack {
                Button(action: {
                    ...
                    FBFirestore.addReviewProf(id: UUID().uuidString, name: userInfo.user.name, email: userInfo.user.email, review: text, timeDate: timestamp, profEmail: professor.email){ (result) in
                            switch result {
...
struct ProfessorReview_Previews: PreviewProvider {
    static var previews: some View {
        ProfessorReview(reviews: testData)
    }
}
